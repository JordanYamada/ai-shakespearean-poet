...
#     self._past_lives = {role : [question, intro, outro]}


# #### Making all our getters
#   @property
#   def name(self):
#     return self._name
#   @name.setter
...
#     new_task = input(f'''================================\n{self.question}\n================================\n>>>>''')
#     self.task = new_task
#     completion = asyncio.create_task(client.chat.completions.create(
#       model="gpt-3.5-turbo",
#       messages=[
...
#   async def ask_Shakepeare(self):
#     response = asyncio.create_task(client.chat.completions.create(
#       model="gpt-3.5-turbo",
#       messages=[
...
#       asyncio.run(self.new_question())
#       asyncio.run(self.new_intro())
#       asyncio.run(self.new_outro())
#       asyncio.run(self.new_farewell())
...
#       ]
#     ))
#     question = await response.choices[0].message.content
...
#       ]
#     ))
#     outro = await response.choices[0].message.content
#     self.outro = outro
...
#       ]
#     ))
#     intro = await response.choices[0].message.content
#     self.intro = intro
#     self.past_responses.append(intro)


# ### Adding a farewell message for later
#   async def new_farewell(self):
#     response = asyncio.create_task(client.chat.completions.create(
#       model="gpt-3.5-turbo",
#       messages=[
#         {"role": "system", "content": f"You are a {self.role}"},
...
#       ]
#     ))
#     farewell = await response.choices[0].message.content
...
#         asyncio.run(self.ask_Shakepeare())
#         self.start()
...
#       case '3':
#         asyncio.run(self.ask_a_question())
#         self.start()
